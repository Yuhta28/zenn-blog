...
nuxt-appコマンドを実行しますと、対話形式にNuxtの設定を行えることができ、簡単にNuxtのプロジェクトを作成することができます。質問の詳しい内容は[こちらのサイト](https://ja.nuxtjs.org/guide/installation/)を参考にして各自の好みに合わしたプロジェクトのひな型を作成します。
...
# DockerHubへのpush
`build_image.yml`で自動pushされるように設定しましたので、ご覧のように私のDockerHubへイメージがpushされています。![](https://storage.googleapis.com/zenn-user-upload/f0fq07sibn7yyf7atacm6z07vabs)
