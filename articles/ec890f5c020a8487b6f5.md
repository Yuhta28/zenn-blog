...
# credstashについて
credstashですが日本語情報が少なく、配布元GitHubからの説明になりますが認証情報の管理および共有システムといえます。
...
Apacheを動かしているサーバーのPythonが2.6系だったのですが、2.6系ですとcredstashのインストールができなかったため、pyenvを用いてPythonを3.6系に変更しました。
その時に環境変数回りで問題があったのか、起動できないという問題がありました。
回避方法として、`passphrase.sh`の中身を以下のように修正したらcredstashが上手く動き、Apacheの再起動に成功しました。
