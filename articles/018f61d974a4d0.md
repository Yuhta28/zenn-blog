# 背景
現在こちらの本を読んでCircle CIについて勉強中です。
https://gihyo.jp/book/2020/978-4-297-11411-4

Circle CIはクラウド環境で実行するイメージしかありませんでしたが、ローカル環境でも実行することは可能で、目的としては`config.yml`の文法チェック、ジョブの実行確認などが挙げられます。
...
初めて見るエラーでしたので、ググりましたが端末再起動という解決策が複数サイトで見つかったので、再起動を行い改めて`root`ユーザで実行してみた所、問題なくインストールすることに成功しました。
...
# CircleCI CLI使用感
`.cirlceci/config.yml`があるカレントディレクトリで`circleci config validate`を実行すると`config.yml`の文法エラーとCircleCIの設定エラーの二つを確認できます。
注：config.yml直下のディレクトリではありません。失敗します（実体験)
```
$ ll .circleci/
合計 12
...
```
設定チェックが問題なければ、`circleci local execute`コマンドを実行することで、ローカル環境でジョブを実行することができます。
...
ワークフローが含まれてますが、技術書の説明で単体のみ実行できると記載がありましたので、一番最初の`test1`だけ実行できるとかと思いましたが、`test1`の実行も失敗してしまいました。
...
技術書では特に触れられていませんでしたが、リファレンスによりますと特定のジョブを実行したい場合、オプション`--job JOB_NAME`をつけることで実行することができると記載されています。
...
後は、gitでいつものようにaddして、commitして、pushすればクラウド上のCirlceCIが動きCIが始まります。

# 感想
私としてはYAMLファイルは読みやすくて好きですが、記述ミスが見落としやすく、エラー発生時の原因調査で時間がかかる場合があります。`config.yml`の変更を行ったら逐次validateを行うことでミスに気づき、時間短縮に繋がりますので積極的に使っていきたいと思います。
また個人的にですが、AnsibleやTerraformのようにCircleCI CLIにも`config.yml`のテンプレート作成コマンドがあれば、便利だなと思いますが現状自分で一から作成する以外方法がない感じですかね🤔
