...
余談ですが、Redshiftも旧世代のインスタンスタイプを使っていましたが、こちらは問題なく作成されました。
...
テンプレートファイルを修正して再度スタックの作成を実行すれば、プロビジョニングが完了しますので出力ページから`OracleJDBCConnectionString`と`RedshiftJDBCConnectionString`の値を控えておきます。
